...
class Drone3Control : public rclcpp::Node
{
public:
    Drone3Control();
...
private:
    ...
    rclcpp::Subscription<px4_msgs::msg::VehicleGlobalPosition>::SharedPtr leadf_gps_sub;
...
Drone3Control::Drone3Control() : Node("displacement_control_drone2")
{
    ...
    lead_gps_sub = create_subscription<px4_msgs::msg::VehicleGlobalPosition>(
        "/px4_2/fmu/out/vehicle_",
        qos_profile,
        std::bind(&Drone3Control::lead_gps_to_ned, this, std::placeholders::_1));

    leadf_gps_sub = create_subscription<px4_msgs::msg::VehicleGlobalPosition>(
        "/px4_1/fmu/out/vehicle_odometry",
        qos_profile,
        std::bind(&Drone3Control::fol_gps_to_ned, this, std::placeholders::_1));

    own_gps_sub = create_subscription<px4_msgs::msg::VehicleGlobalPosition>(
        "/px4_3/fmu/out/vehicle_odometry",
        qos_profile,
        std::bind(&Drone3Control::own_gps_to_ned, this, std::placeholders::_1));


    own_odom_sub = create_subscription<px4_msgs::msg::VehicleOdometry>(
        "/px4_2/fmu/out/vehicle_odometry",
        qos_profile,
        std::bind(&Drone3Control::own_odom_sub, this, std::placeholders::_1));

    // Timer to send setpoints periodically
    timer_ = create_wall_timer(100ms, std::bind(&Drone3Control::start_run, this));

    desired_own_pos << -4.0f, 0.0f, 0.0f;
    desired_lead_pos << 0.0f, 0.0f, 0.0f;
}

Eigen::Vector3f Drone3Control::collision_logic()
{

    std::vector<Eigen::Vector3f> fol_list = {leadf_odom_pos, lead_odom_pos};
...
void  Drone3Control::lead_gps_to_ned(const px4_msgs::msg::VehicleGlobalPosition::SharedPtr lead_gps_msg)
{
...
void Drone3Control::fol_gps_to_ned(const px4_msgs::msg::VehicleGlobalPosition::SharedPtr fol_gps_msg)
{
    Eigen::Vector3d fol_gps_pos = {fol_gps_msg -> lat, fol_gps_msg -> lon, static_cast<double>(fol_gps_msg -> alt)};

    leadf_odom_pos << gps_to_ned(fol_gps_pos);
    gps_leadf_received = true;
}

void Drone3Control::own_gps_to_ned(const px4_msgs::msg::VehicleGlobalPosition::SharedPtr own_gps_msg)
{
...
void Drone3Control::own_odom_func(const px4_msgs::msg::VehicleOdometry::SharedPtr odom_own_msg)
{
...
// func count 1
void Drone3Control::trajectory_logic()
{   
    ...
    if (!z_sync_phase)
    {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                             ...
                             leadf_odom_pos.x(), leadf_odom_pos.y(), leadf_odom_pos.z(),
...
void Drone3Control::offboard_control_mode()
{
...
void Drone3Control::set_offboard_command()
{
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<Drone3Control>());
    rclcpp::shutdown();
    return 0;
}